...
export default {
    ...
    methods:{
        ...
        //최신
        async reviewListRecent () {
            await axios.get('http://localhost:8080/review/reviewListRecent')
            .then((res) => {
        ...
        infiniteHandler($state) {
            const EACH_LEN = 12
            axios.get('http://localhost:8080/review/reviewListPageRcent?page='+this.loadNum)
            .then(res => {
                //console.log("샐행은 되니?")
                console.log(res.data+"무한로딩중~.~"+this.loadNum)
